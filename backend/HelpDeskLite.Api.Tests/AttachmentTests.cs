...
public sealed class AttachmentTests(ApiFactory factory):IClassFixture<ApiFactory>
{
    ...
    [Fact]public async Task Unsupported_file_type_is_rejected(){using var body=Image("notes.txt","text/plain");var response=await factory.ClientFor(ApiFactory.EmployeeId,AppRoles.Employee).PostAsync("/api/tickets/1/attachments",body);Assert.Equal(HttpStatusCode.BadRequest,response.StatusCode);}
