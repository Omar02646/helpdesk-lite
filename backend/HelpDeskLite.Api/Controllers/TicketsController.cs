...
public sealed class TicketsController(TicketService service):ControllerBase {
    ...
    [HttpGet("my"),Authorize(Roles=AppRoles.Employee)]public Task<List<TicketDto>> My([FromQuery]TicketQuery query,CancellationToken ct)=>service.QueryAsync(query,UserId,false,ct);
    ...
    [HttpGet,Authorize(Roles=AppRoles.SupportAgent+","+AppRoles.Manager)]public Task<List<TicketDto>> All([FromQuery]TicketQuery query,CancellationToken ct)=>service.QueryAsync(query,null,false,ct);
    ...
    [HttpPatch("{id:int}/status"),Authorize(Roles=AppRoles.SupportAgent)]public async Task<IActionResult> Status(int id,ChangeStatusRequest request,CancellationToken ct){try{await service.UpdateStatusAsync(id,request.Status,UserId,ct);return NoContent();}catch(KeyNotFoundException){return NotFound();}}
